...
local folder_icon_closed = vim.g.have_nerd_font and "\u{f07b} " or "📁 "
local folder_icon_open = vim.g.have_nerd_font and "\u{f07c} " or "📂 "

local function stat_hl(stat_type, row_hl)
	local suffix = HL_TINT_SUFFIX[row_hl]
	if suffix then
		return "GitCompareStat" .. stat_type .. suffix
	end
...
local function win_type(win)
	...
	if ft == "git_compare_panel" then
		...
		if bufnr == state.origin_buf then
			return "origin"
		end
		if bufnr == state.accept_buf then
			return "accept"
		end
	end
	return nil
...
local function render_tree(root, origin_status, accept_status, dir_open, commit)
	...
	local function walk(node, pfx)
		...
		for i, child in ipairs(children) do
			...
			-- Highlight range covers the folder icon + name (byte offsets in the line).
			-- connector = "└ "/"├ " (4 bytes each); icon = arrow (4 bytes) + folder glyph.
			if child.is_dir then
				local col_s = #pfx + #connector
				local col_e = col_s + #icon + #child.name
...
local function fl_hash(fl, dir_open_state)
	local keys = {}
	for k in pairs(fl.new) do
		table.insert(keys, "n:" .. k)
	end
	for k in pairs(fl.modified) do
		table.insert(keys, "m:" .. k)
	end
	table.sort(keys)
	local open_keys = {}
	for k, v in pairs(dir_open_state) do
		if v then
			table.insert(open_keys, k)
		end
	end
...
local function do_refresh()
	...
	if state.origin_buf and vim.api.nvim_buf_is_valid(state.origin_buf) and new_origin_hash ~= _sidebar_hash.origin then
	...
	if state.accept_buf and vim.api.nvim_buf_is_valid(state.accept_buf) and new_accept_hash ~= _sidebar_hash.accept then
...
local function create_panels()
	...
	attach_keymaps(origin_buf, "origin", function()
		return state.origin_node_data
	end, function()
		return state.origin_dir_open
	end)
	attach_keymaps(accept_buf, "accept", function()
		return state.accept_node_data
	end, function()
		return state.accept_dir_open
	end)
	...
	vim.schedule(function()
		...
		vim.defer_fn(function()
			...
			if target_buf and lnum < HEADER_LINES + 1 then
				lnum = HEADER_LINES + 1
			end
...
function M.setup()
	...
	local function smart_left()
		...
		if
			cur_ft ~= "NvimTree"
			and cur_ft ~= "git_compare_panel"
			and _last_left_win
			and vim.api.nvim_win_is_valid(_last_left_win)
		then
	...
	vim.keymap.set(
		...
		"<C-w><C-h>",
		smart_left,
		{ noremap = true, silent = true, desc = "Go to last focused left panel" }
	)
end
